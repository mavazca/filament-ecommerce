                    ...
                    <div class="mt-1 flex items-center gap-x-2">
                        @php
                            ...
                            if($order->payment_status == 'failled'){
                                $payment_status = '<span class="bg-re-600 py-1 px-3 rounded text-white shadow">Failled</span>';
                            }
                        @endphp
                    ...
                    <tbody>
                        @foreach($order->items as $item)
                            <tr wire:key="{{ $item->id }}">
                                ...
                                <td class="py-4">{{ Number::currency($item->unit_amount, 'BRL', 'pt_BR') }}</td>
                                <td class="py-4">
                                    <span class="text-center w-8">{{ $item->quantity }}</span>
                                </td>
                                <td class="py-4">{{ Number::currency($item->total_amount, 'BRL', 'pt_BR') }}</td>
                            </tr>
                        @endforeach
                    </tbody>
                </table>
        ...
        <div class="md:w-1/4">
            <div class="bg-white rounded-lg shadow-md p-6">
                ...
                <div class="flex justify-between mb-2">
                    <span>Subtotal</span>
                    <span>{{ Number::currency($order->grand_total, 'BRL', 'pt_BR') }}</span>
                </div>
                <div class="flex justify-between mb-2">
                    <span>Taxes</span>
                    <span>{{ Number::currency(0, 'BRL', 'pt_BR') }}</span>
                </div>
                <div class="flex justify-between mb-2">
                    <span>Shipping</span>
                    <span>{{ Number::currency(0, 'BRL', 'pt_BR') }}</span>
                ...
                <div class="flex justify-between mb-2">
                    ...
                    <span class="font-semibold">{{ Number::currency($order->grand_total, 'BRL', 'pt_BR') }}</span>
                </div>
